class SECRlpEncode {
  ...
  _jsonToArray (input) {
    ...
    Object.keys(input).forEach(function (key) {
      if (typeof input[key] === 'number') {
        input[key] = input[key].toString('base64')
      }
  ...
  _combineKeyValue (decodeResult, jsonKeyArray) {
    ...
    for (i = 0; i < jsonKeyArray.length; i++) {
      ...
      if ((typeof decodeResult[i - indexDiff] === 'string') && (typeof jsonKeyArray[i] === 'string')) {
      ...
      } else if ((decodeResult[i - indexDiff] instanceof Array) && (typeof jsonKeyArray[i] === 'string')) { // {'a': {'b': 'c'}} => [a, b] and [c]
        // console.log('second loop')
        jsonArray.push('"', jsonKeyArray[i], '": ')
        indexDiff++
      } else if ((decodeResult[i - indexDiff] instanceof Array) && (jsonKeyArray[i] instanceof Array)) { // nested array and dict, e.g. {{}} or {[]} or [{}] or [[]]
        ...
        if ((jsonKeyArray[i][0] === '1') && (decodeResult[i - indexDiff][0] === '1')) { // it's array
          ...
          jsonArray.push('],')
        } else if ((jsonKeyArray[i][0] === '2') && (decodeResult[i - indexDiff][0] === '2')) { // {{'a': 'b'}} => [[a]], and [[b]] or [{'a': 'b'}] => [[a]], and [[b]]
          jsonArray.push('{')
          jsonArray.push(this._combineKeyValue(decodeResult[i - indexDiff].slice(1), jsonKeyArray[i].slice(1)))
          jsonArray.push('},')
        } else {
